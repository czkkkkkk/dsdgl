...
void OrderedHashTable<IdType>::FillWithDuplicates(
  ...
  IdArray item_prefix_array = MemoryManager::Global()->Empty("ITEM_PREFIX", {int64_t(num_input+1)}, dtype, ctx_);
  IdType *item_prefix = item_prefix_array.Ptr<IdType>();
  ...
  IdArray workspace_array = MemoryManager::Global()->Empty("WORKSPACE", {int64_t(workspace_bytes/sizeof(IdType)+1)}, dtype, ctx_);
  void *workspace = workspace_array.Ptr<IdType>();
