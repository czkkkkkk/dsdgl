...
class DeviceNodeMapMaker {
  ...
  */
  void Make(
    ...
    // node_maps里记录了global vid -> local vid的映射（local vid是连续的）
    ...
    // unique rhs nodes
    // 生成 global vid -> local vid的映射，注意，如果lhs包含rhs的话，可以保证相同的global vid
    // 映射到相同的local vid，因为复制的时候rhs中的vid放在了lhs前端
...
ToBlockGPU(
  ...
  // 完成 global vid 到 local vid 的映射
  CHECK_EQ(src_nodes.size(), 1);
  src_nodes[0] = Partition(src_nodes[0], min_vids);
  maker.Make(
      src_nodes,
      rhs_nodes,
      &node_maps,
...
ToBlock<kDLGPU, int32_t>(
    ...
    bool include_rhs_in_lhs,
    IdArray min_vids) {
  return ToBlockGPU<int32_t>(graph, rhs_nodes, include_rhs_in_lhs, min_vids);
}
...
ToBlock<kDLGPU, int64_t>(
    ...
    bool include_rhs_in_lhs,
    IdArray min_vids) {
  return ToBlockGPU<int64_t>(graph, rhs_nodes, include_rhs_in_lhs, min_vids);
}
