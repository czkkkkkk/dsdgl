...
class WebUKDataset(DGLDataset):
  raw_dir = '/data/webgraph/'
  url = '/data/webgraph/uk-2006-05.graphtxt'

  def __init__(self, raw_dir=None, force_reload=False,
               verbose=False, transform=None):
    self.num_classes = 3
    super(WebUKDataset, self).__init__(name='webuk',
                                       url=WebUKDataset.url,
                                       raw_dir=WebUKDataset.raw_dir,
  ...
  def process(self):
    ...
    col = []
    num_nodes = 0
    with open(WebUKDataset.url, 'r') as f:
      line = f.readline()
      num_nodes = int(line)
      num_edges = 0
      cur_node = 0
